...
class HorizontalCarouselRecyclerView(
    ...
    private val activeColor by lazy { ContextCompat.getColor(context, R.color.PortPrimaryLight) }
    ...
    fun <T : ViewHolder> initialize(newAdapter: Adapter<T>) {
       ...
       // (layoutManager as LinearLayoutManager).reverseLayout = true
        ...
        //(layoutManager as LinearLayoutManager).scrollToPositionWithOffset(0,1)
       // (layoutManager as LinearLayoutManager).stackFromEnd = true
        ...
        newAdapter.registerAdapterDataObserver(object : RecyclerView.AdapterDataObserver() {
            override fun onChanged() {
                post {
                    getChildAt(0)?.let {
                        val sidePadding = (width / 2) - (it.width / 2)
    ...
    private fun onScrollChanged() {
        post {
            (0 until childCount).forEach { position ->
                ...
                val scaleValue =  getGaussianScale(childCenterX, 1f, 0.9f, 200.toDouble())*0.618F
    ...
    private fun getGaussianScale(
        ...
        return (Math.E.pow(
            -(childCenterX - recyclerCenterX.toDouble()).pow(2.0) / (6 * spreadFactor.pow(2.0))
